...
class APU:
    ...
    def __post_init__(self):
        ...
        self.rendimiento = db.xlookup(
            'Partidas', self.partida_ID, 'ID', 'Rendimiento')
        self.insumo_unidad_ID = db.xlookup(
            'Insumos', self.insumo_ID, 'ID', 'Unidad')
        self.insumo_unidad_code = db.xlookup(
            'Unidades', self.insumo_unidad_ID, 'ID', 'Code')
        ...
        self.partida_nombre = db.xlookup(
            'Partidas', self.partida_ID, 'ID', 'Nombre')
        self.insumo_nombre = db.xlookup(
            'Insumos', self.insumo_ID, 'ID', 'Nombre')
    ...
    def add(self):
        ...
        try:
            ...
            """
            cursor.execute(
                sql, (self.partida_ID, self.insumo_ID, self.cantidad))
            conn.commit()
        except sqlite3.Error as e:
            if "UNIQUE constraint failed" in str(e):
                raise cs.SQLiteError(f'El insumo "{self.insumo_nombre}" ya existe en la partida "{
                                     self.partida_nombre}".')
            else:
